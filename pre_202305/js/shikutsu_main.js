...
$.getJSON(json_url, function (config) {
  set_config(config);
  //言語設定　watanabe
  func_change_language(change_language);
  ...
  $.ajax({
    ...
    if (data.features.length > 0) {
      ...
      alert(change_language["alert-anonymousPanel-invalidurl"]);//watanabe
...
require([
  ...
  function sign_out() {
    ...
    alert(change_language["alert-maindiv-signout"]);//watanabe
    // サインアウト後最初の画面に戻す
  ...
  identityManager.checkSignInStatus(portalUrl).then(function () {
    ...
    $.ajax({
      ...
      $.ajax({
        ...
        alert(change_language["alert-failureurl"]);//watanabe
      });
    }).fail(function (data) {
      alert(change_language["alert-failureurl"]);//watanabe
    });
  ...
  function displayForm() {
    ...
    portal.load().then(function () {
      ...
      portal.user.fetchGroups().then(function (fetchItemResult) {
        ...
        if (match == undefined) {
          identityManager.destroyCredentials();
          alert(change_language["alert-user"]);
  ...
  function initForm() {
    ...
    var viewAction = {
      title: change_language["maindiv-map-button1"],
      id: "view-this",
      className: "esri-icon-search"
    };
    var deleteAction = {
      title: change_language["maindiv-map-button2"],
      id: "delete-this",
    ...
    historyview.when(function () {
        ...
        content: [
          {
            ...
            fieldInfos: [{
              fieldName: "Naiyo",
              label: change_language["maindiv-map-label1"]
            }, {
              fieldName: "Jusho",
              label: change_language["maindiv-map-label2"]
            }, {
              fieldName: "Bikou",
              label: change_language["maindiv-map-label3"]
            }]
          },
        ]
      };
      ...
      const labelClass = {
        labelExpressionInfo: {
          expression: "\"" + change_language["maindiv-map-label4"] + "\\n\" + Text($feature." + create_date_field + ", 'Y/MM/DD HH:mm')"
        },
  ...
  $('#show_form').click(function () {
    ...
    each_switch_kaisha_id_setting(switch_naiyo_kaisha_id_list, function (setting) {
      func_default_value_setting(setting.default_value[lang], $("#formDiv"));
    });
    change_display(2);
  });
  ...
  //マップの表示・非表示切り替え　　watanabe
  $('.locate_conpact').click(function () {
    $('#mapviewDiv').toggle(function () {
      if ($(this).is(':visible')) {
        $(".locate_conpact").text(change_language["formdiv-button-close"]);
      } else {
        $(".locate_conpact").text(change_language["formdiv-button-open"]);
      }
    });
  ...
  $('#sendbtn').click(function () {
    ...
    if (kaishaid == "") {
      alert(change_language["alert-companyid"]);//watanabe
      return;
    }
    ...
    if (title == "") {
      alert(change_language["alert-title"]);//watanabe
      $('#title').focus();
      return;
    }
    ...
    if (ngCharaFlg) {
      alert(change_language["alert-ngchara"]);//watanabe
      return;
    }
    ...
    if (size > upload_limit_total_file_size) {
      alert(change_language["alert-size1"] + func_file_size(upload_limit_total_file_size) + change_language["alert-size2"]); //watanabe
      return;
    }

    //現在地の指定チェック
    if (latitude == null || longitude == null) {
      alert(change_language["alert-location"]); //watanabe
      return;
    }
    ...
    if (!checked) {
      alert(change_language["alert-deliverables"]);
      return;
    }

    $('#sendbtn').html(change_language["formdiv-button-sending"]);//watanabe
  ...
  $('#edit_send').click(function () {
    ...
    if (!checked) {
      alert(change_language["alert-deliverables"]);//watanabe
      return;
    }
  ...
  $("#file_select").change(function () {
    // ファイル選択
    upload_files(this.files, "#file_table", "#file_select", "#file_label", change_language["formdiv-fileselection-argument"], {
      // Exif情報処理
      hasExif: function (exifs) {
        if (user_setting.toko == "1" && !isMobile && exifs.length > 0) { // ユーザー設定がExifの時かつPC端末の場合
          if (confirm(change_language["formdiv-confirm-message"]) != false) { //watanabe
            // 地図を画像の位置に移動
            view.goTo({
  ...
  async function upload_files(files, selector_table, selector_select, selector_label, caption, callbacks) {
        ...
        return new $.Deferred(function (defer) {
          ...
          if (callbacks.upload_file_check_start != undefined) {
            ...
            if (callbacks.create_row != undefined) {
              ...
              $tr.html("<div class='no'></div>" +
                ...
                "</div>" +
                "<div class='delete'><input type='button' value='" + change_language["formdiv-button-delete"] + "' class='btn-del' /><input type='hidden' class='id' /><input type='hidden' class='size' /><input type='hidden' class='name' /></div>"); //watanabe
            }
            $tr.find("div.no").text(rowNo);
            $tr.find("div.name").text(file.name);
            $tr.find("div.size").text(func_file_size(file.size));
            $tr.find("div.status").text(change_language["formdiv-filetable-status1"]);//watanabe
            // 削除ボタンクリック
            $tr.find(".btn-del").click(function () {
              ...
              // カウント表示の更新
              if (select_count > 0) {
                if (selector_label.length > 0) $(selector_label).text(select_count + change_language["formdiv-filetable-message2"]);
              }
              else {
                if (selector_label.length > 0) $(selector_label).text("");
              }
          ...
          if (upload_file_type_white_list.length > 0 && !func_check_wildcard(file.type, upload_file_type_white_list) &&
            ...
            if (callbacks.upload_file_check_ng != undefined) {
              callbacks.upload_file_check_ng(i, file, change_language["formdiv-filetable-status2"]);
            }
            else {
              $tr.find(".status").html("<span style='color:red;'>" + change_language["formdiv-filetable-status2"] + "</span>");
            }
            cancel = true;
          }
          if (upload_extension_white_list.length > 0 && !func_check_wildcard(file.name, upload_extension_white_list) &&
            ...
            if (callbacks.upload_file_check_ng != undefined) {
              callbacks.upload_file_check_ng(i, file, change_language["formdiv-filetable-status3"]);
            }
            else {
              $tr.find(".status").html("<span style='color:red;'>" + change_language["formdiv-filetable-status3"] + "</span>");
            }
            cancel = true;
          }
          ...
          if (file.size == 0) {
            if (callbacks.upload_file_check_ng != undefined) {
              callbacks.upload_file_check_ng(i, file, change_language["formdiv-filetable-status4"]);
            }
            else {
              $tr.find(".status").html("<span style='color:red;'>" + change_language["formdiv-filetable-status4"] + "</span>");
            }
            cancel = true;
          }
          if (file.size > upload_limit_file_size) {
            if (callbacks.upload_file_check_ng != undefined) {
              callbacks.upload_file_check_ng(i, file, change_language["formdiv-filetable-status5"]);
            }
            else {
              $tr.find(".status").html("<span style='color:red;'>" + change_language["formdiv-filetable-status5"] + "</span>");
            }
            cancel = true;
          }
          if (!cancel) {
            if (callbacks.upload_ready != undefined) {
              ...
              $tr.find(".status").html("<span style='color:black;'>" + change_language["formdiv-filetable-status6"] + "</span>");
            }
            // ファイル読み込み開始
            read_file(file).then(function (args) {
              ...
              if (callbacks.upload_file_readed != undefined) {
                ...
                $tr.find(".status").html("<span style='color:blue;'>" + change_language["formdiv-filetable-status7"] + "</span>");
              }
              ...
              request_ajax(param).then(function (args) {
                ...
                if (!cancel && registed_data != undefined && (registed_data.success || false)) {
                  ...
                  if (callbacks.upload_file_registed != undefined) {
                    ...
                    $tr.find(".status").html("<span style='color:blue;'>" + change_language["formdiv-filetable-status8"] + "</span>");
                    $tr.find(".status").html("<span style='color:green;'>" + change_language["formdiv-filetable-status9"] + "</span><meter min='0' max='100' value='0'></meter>");
                  }
                  // Blobデータを指定サイズで分割
                  // 分割アップロード開始
                  upload_split_blob(blob, blob_chunk_size, {
                    ...
                    uploading: function (args) {
                      ...
                      if (!args.cancel && args.data != undefined && (args.data.success || false)) {
                        ...
                        if (callbacks.uploading != undefined) {
                          ...
                          $tr.find(".status").html("<span style='color:green;'>" + change_language["formdiv-filetable-status10"] + Math.ceil(((args.index + 1) / args.division_count) * 100) + "%)</span><meter min='0' max='100' value='" + Math.ceil(((args.index + 1) / args.division_count) * 100) + "'></meter>");
                        }
                      }
                      else {
                        args.cancel = true;
                    ...
                    if (callbacks.upload_commit_start != undefined) {
                      ...
                      $tr.find(".status").html("<span style='color:blue;'>" + change_language["formdiv-filetable-status11"] + "</span>");
                    }
                    ...
                    request_ajax(param).then(function (args) {
                      ...
                      if (!cancel && args != undefined && (args.success || false)) {
                        ...
                        if (callbacks.upload_commit_applied) {
                          ...
                          $tr.find(".status").html("<span style='font-weight:bold;color:blue;'>" + change_language["formdiv-filetable-status12"] + "</span>");
                        ...
                        if (callbacks.upload_commit_failed != undefined) {
                          callbacks.upload_commit_failed(i, file, change_language["formdiv-filetable-status13"]);
                        }
                        else {
                          $tr.find(".status").html("<span style='color:red;'>" + change_language["formdiv-filetable-status13"] + "</span>");
                        }
                        defer.reject();
                      }
                    }).fail(function () {
                    ...
                    if (callbacks.upload_failed != undefined) {
                      callbacks.upload_failed(i, file, change_language["formdiv-filetable-status14"]);
                    }
                    else {
                      $tr.find(".status").html("<span style='color:red;'>" + change_language["formdiv-filetable-status14"] + "</span>");
                    }
                    defer.reject();
                  });
                }
                  ...
                  if (callbacks.upload_file_regist_failed != undefined) {
                    callbacks.upload_file_regist_failed(i, file, change_language["formdiv-filetable-status15"]);
                  }
                  else {
                    $tr.find(".status").html("<span style='color:red;'>" + change_language["formdiv-filetable-status15"] + "</span>");
                  }
                  defer.reject();
                }
              }).fail(function () {
              ...
              if (callbacks.upload_file_read_error) {
                callbacks.upload_file_read_error(i, file, change_language["formdiv-filetable-status16"]);
              }
              else {
                $tr.find(".status").html("<span style='color:red;'>" + change_language["formdiv-filetable-status16"] + "</span>");
              }
              defer.reject();
            });
          }
    ...
    if (select_count > 0) {
      if (selector_label.length > 0) {
        let messages = {
          ja: select_count + "件の" + caption + "が選択されました",
          en: select_count + " " + caption + " " + "selected"
        }
        $(selector_label).text(messages[lang]);//watanabe99
      }
  ...
  function append_attachment_file(oid, files) {
    ...
    upload_files(files, "#attachmentlistDiv", "#addattachment-select", "", change_language["viewformdiv-fileselection-argument"], {
      begin: function () {
  ...
  function form_clear() {
    ...
    $('#sendbtn').html(change_language["formdiv-button-send"]);//watanabe
  }
  ...
  function saveUserConf() {
    ...
    $("#saved").html("<label>" + change_language["userconf-message"] + "</label>");
    //$('.returnmenu').click();
  }
  ...
  function changePasswordAction() {
    ...
    if (currentPwd.length == 0) {
      list.push(change_language["changediv-message1"]);
    }
    if (newPwd.length == 0) {
      list.push(change_language["changediv-message2"]);
    }
    if (rePwd.length == 0) {
      list.push(change_language["changediv-message3"]);
    }
    if (currentPwd == newPwd && currentPwd.length != 0 && newPwd.length != 0) {
      list.push(change_language["changediv-message4"]);
    }
    if (newPwd != rePwd) {
      list.push(change_language["changediv-message5"])
    }
    if (list.length == 0) {
      var form = new FormData();
      form.set('f', 'json');
      ...
      $.ajax({
        ...
        if (data.error) {
          $('#warnDiv').html('<ul><li>' + change_language["changediv-message6"] + '</li></ul>');
        }
        else {
        ...
        // console.log(xhr);
        $('#warnDiv').html('<ul><li>' + change_language["changediv-message7"] + '</li></ul>');
      });
    }
    else {
      html += '<ul>';
  ...
  function changePassword(pwd) {
    ...
    $.ajax({
      ...
      if (data.success) {
        alert(change_language["alert-changepass"]);
        document.location.reload()
      }
      else {
        // console.log(data)
        $('#warnDiv').html('<ul><li>' + change_language["warndiv-message"] + '</li></ul>');
      }
    }).fail(function (xhr) {
      $('#warnDiv').html('<ul><li>' + change_language["warndiv-message"] + '</li></ul>');
      // console.log(xhr);
  ...
  $("#attachment_file").on("change", function () {
    ...
    upload_files(this.files, "#attachment_list_user .file-list", "#attachment_file", "", null, {
      ...
      upload_file_check_start: function (idx, file) {
        ...
        $("<div></div>").addClass("title").text("ファイル名").appendTo($col);
        $("<div></div>").addClass("value").text(file.name).appendTo($col);
        $col = $("<div></div>").addClass("upload-status flex-box").appendTo($row);
        $("<div></div>").addClass("title").text("アップロード状況").appendTo($col);
        $("<div></div>").addClass("value").text("登録開始").appendTo($col);
      },
      // ファイルチェックNG
      ...
      upload_ready: function (idx) {
        let $div = $("#uploading_row" + idx).find(".upload-status .value");
        $div.empty();
        $("<span style='color:blue;'></span>").text(change_language["uploadinfotext6"]).appendTo($div);
      },
      // アップロード開始
      upload_file_readed: function (idx, file) {
        let $div = $("#uploading_row" + idx).find(".upload-status .value");
        $div.empty();
        $("<span style='color:blue;'></span>").text(change_language["uploadinfotext5"]).appendTo($div);
      },
      // ファイル読み込みエラー
      ...
      upload_file_registed: function (idx, file) {
        let $div = $("#uploading_row" + idx).find(".upload-status .value");
        $div.empty();
        $("<span style='color:green;'>" + change_language["uploadinfotext4"] + "(0%)</span><meter min='0' max='100' value='0'></meter>").appendTo($div);//watanabe
      },
      // アップロードファイル登録失敗
      ...
      uploading: function (idx, file, prog) {
        let $div = $("#uploading_row" + idx).find(".upload-status .value");
        $div.find("span").text(change_language["uploadinfotext4"] + "(" + prog + "%)");//watanabe
      ...
      upload_commit_start: function (idx, file) {
        let $div = $("#uploading_row" + idx).find(".upload-status .value");
        $div.empty();
        $("<span style='color:blue;'></span>").text(change_language["uploadinfotext3"]).appendTo($div);//watanabe
      },
      // アップロードファイル適用完了
            ...
            await request_ajax({
              ...
              if (data != undefined && data.addResults != undefined && data.addResults.length > 0 && data.addResults[0].success) {
                ...
                await request_ajax({
                  ...
                  let $div = $("#uploading_row" + idx).find(".upload-status .value");
                  $div.empty();
                  $("<span style='font-weight:bold;color:blue;'></span>").text(change_language["uploadinfotext1"]).appendTo($div);//watanabe
                  defer.resolve();
                }).fail(function (data) {
                  let $div = $("#uploading_row" + idx).find(".upload-status .value");
                  $div.empty();
                  $("<span style='font-weight:bold;color:red;'></span>").text(change_language["uploadinfotext2"]).appendTo($div);//watanabe
                  defer.reject();
                });
              }
              else {
...
var historyTable = {
  ...
  loadHead: function () { //watanabe99
    let strRowHead = document.getElementById("tableHead");
    strRowHead.innerHTML = `<tr class="row-head">
          <th class="sys-change-content">${change_language["maindiv-table-header1"]}</th>
          <th class="sys-change-content">${change_language["maindiv-table-header2"]}</th>
          <th class="sys-change-content">${change_language["maindiv-table-header3"]}</th>
          <th class="sys-change-content">${change_language["maindiv-table-header4"]}</th>
          <th class="sys-change-content">${change_language["maindiv-table-header5"]}</th>
          <th class="sys-change-content">${change_language["maindiv-table-header6"]}</th>
        </tr>`;

    // 特定会社ID時設定置き換え
    each_switch_kaisha_id_setting(switch_naiyo_kaisha_id_list, function (setting) {
      func_change_content_replace(setting["change_content"][lang], $("#tableHead"));
    });
  },
    ...
    $.ajax({
      ...
      if (features != undefined) {
        for (var i = 0; i < features.length; i++) {
          ...
          //watanabe
          tr_html += '<tr>';
          tr_html += '<td data-label=' + change_language["maindiv-table-header1"] + '>' + title + '　</td>';
          tr_html += '<td data-label=' + change_language["maindiv-table-header2"] + '>' + str_createdate + '　</td>';
          tr_html += '<td data-label=' + change_language["maindiv-table-header3"] + '>' + naiyo + '　</td>';
          tr_html += '<td data-label=' + change_language["maindiv-table-header4"] + '>' + jusho + '　</td>';
          tr_html += '<td data-label=' + change_language["maindiv-table-header5"] + '>' + bikou + '　</td>';
          tr_html += '<td data-label=' + change_language["maindiv-table-header6"] + '>'
          tr_html += '<input id="view" type="button" value="' + change_language["maindiv-table-button1"] + '" onclick="historyTable.viewForm(' + objectid + ')"/>';
          tr_html += '<input id="del" type="button" value="' + change_language["maindiv-table-button2"] + '" onclick="historyTable.deleteRow(' + objectid + ')"/>';
          tr_html += '</td>';
          tr_html += '</tr>';
  ...
  viewForm: function (objectid) {
    ...
    $.ajax({
      ...
      if (features.length == 0) {
        alert(change_language["alert-upload-nodata"]); //watanabe
        this.refreshRow();
        return;
      }
        ...
        if (keywords.split("|").indexOf("ManagementSystemUpload") != -1) {
          ...
          for (var j = 0; j < request_filetype_setting.length; j++) {
            if (func_request_filetype_setting_exists_keyword(request_filetype_setting, keywords, j)) {
              var html = "<span>" + att_name + '</span><div><a target="_blank" rel="noopener noreferrer"  href="' + att_url + '">' + request_filetype_setting[j].link_text[lang] + '</a></div>';
              // 複数存在する場合は前回要素を削除
          ...
          att_html += att_name + '<br/><a target="_blank" rel="noopener noreferrer"  href="' + att_url + '">' + change_language["viewformdiv-link-download"] + '</a><br/>';
        }
        //watanabe
        att_html += '<input class="delbtn " type="button" id="delBtn' + att_id + '" value="' + change_language["viewformdiv-button-delete"] + '" onclick="historyTable.deleteattachent(' + objectid + ', ' + att_id + ')"/>';
        att_html += '<br/><br/>'
      }
  ...
  deleteRow: function (objectid) {
    ...
    if (!window.confirm(change_language["maindiv-delete-confirm"])) { //watanabe
      return;
    }
...
function set_config(config) {
  ...
  // 文言置き換え用
  change_language = config.languages[lang];
  if (typeof (change_language) == "undefined") {
    change_language = config.languages["ja"];
  }
  // アップロードファイルサイズ単一上限
  ...
  // 特定会社ID時の考慮
  each_switch_kaisha_id_setting(switch_naiyo_kaisha_id_list, function (setting) {
    list_order_select_item = setting.list_order_select_item[lang];
  });
...
function create_request_filetype_check(settings, $parent, name, id_initial, className) {
  for (var i = 0; i < settings.length; i++) {
    if (settings[i].visible) {
      ...
      var $label = $("<label for='" + id_initial + settings[i].field_name + "'></label>").text(settings[i].checkbox_label[lang]).appendTo($div);
 ...
 * @param {Object[]} settings 
 */
function func_change_language(settings) {
  for (let obj in settings) {//設定ファイルの言語オブジェクトをobjに取り出す
    if (obj == "anonymouspanel-button") {
      console.log(obj);
    }
    let $elem = $("." + obj);//設定ファイルの言語オブジェクトのクラス名に.をつけた値を代入
    ...
    $.ajax({
      ...
      for (let attachmentGroup of attachmentGroups) {
        ...
        if (attachmentInfos.length > 0) {
          ...
          //if (attrib["KanriUpload"] != undefined){
          let $list = null;
          // 管理アップロード+ユーザー対象アップロード
          if (attrib["KanriUpload"] != undefined && attrib["KanriUpload"] == "1" && ((attrib["RiyoshaID"] == user && attrib["RiyoshaID"] != creator) || (userLicenseType !== "Editor" && attrib["RiyoshaID"] != creator))) {
            $list = $list_user;
          }
          // 管理アップロード+管理者アップロード
          ...
          if ($list != null) {
            ...
            $("<div></div>").addClass("value").text(formatDate(new Date(attrib["CreationDate"]), 'yyyy/MM/dd HH:mm')).appendTo($col);
            ...
            let $button = $("<input type='button' value='" + change_language["button-download"] + "' />").appendTo($col);
            $button.on("click", () => {
              // let href = url_push_attachment + "/" + layer_id + "/" + attrib["OBJECTID"] + "/attachments/" + attachmentInfo["id"] + "?token=" + token;
              // let $a = $('<a target="_blank" rel="noopener noreferrer"></a>').prop("href", href).prop("download", attachmentInfo["name"]).text("ダウンロード").appendTo($col);
              // let $button = $("<input type='button' value='ダウンロード' />").appendTo($col);
              // ダウンロード
              let href = url_push_attachment + "/" + layer_id + "/" + attrib["OBJECTID"] + "/attachments/" + attachmentInfo["id"] + "?token=" + token;
              window.open(href, "_blank");
              //download_file(href, attachmentInfo["name"]);
              return false;
            });
            let $button_delete = null;
            if (attrib["KanriUpload"] != undefined && attrib["KanriUpload"] == "1") {
              if (userLicenseType === "Creator") {
                $button_delete = $("<input type='button' value='" + change_language["button-delete"] + "' />").appendTo($col);//watanabe
              }
            }
            else {
              $button_delete = $("<input type='button' value='" + change_language["button-delete"] + "' />").appendTo($col);//watanabe
            ...
            if ($button_delete != null) {
              $button_delete.on("click", () => {
                if (!window.confirm(change_language["uploadattachment-delete-confirm"])) {//watanabe
                  return;
                }
                form_disabled(true);
                let form = new FormData();
